...
extension Mission {
    func toRemote() -> OutboundRemoteMission? {
        ...
        let imageFileName: String? = switch state {
            case .draft: nil
            case let .publishing(imagePath): MissionUtils.Image.extractFileNameFromPath(path: imagePath)
            case let .published(imageUrl): MissionUtils.Image.extractFileNameFromUrl(url: imageUrl)
            case let .error(imagePath): MissionUtils.Image.extractFileNameFromPath(path: imagePath)
        }
    ...
    func toMission(getImagePath: (String) -> String?) -> Mission? {
        ...
        let state: Mission.MissionState = switch missionState {
            ...
            case _ where missionState == Mission.MissionState.MissionStateType.published.rawValue:
                    .published(imageUrl: MissionUtils.Image.formatUrl(fileName: missionImageFileName))
...
extension InboundRemoteMission {
    func toMission() -> Mission {
        ...
        return Mission(
            ...
            state: Mission.MissionState.published(imageUrl: MissionUtils.Image.formatUrl(fileName: missionImageFileName))
        )
    }
}
